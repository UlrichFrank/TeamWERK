...
Bank-/SEPA-PII wird ausschließlich clientseitig (im Browser) verschlüsselt (Zero-Knowledge, Modell B). Der Server speichert nur Ciphertext + gewrappte Data-Keys und besitzt zu keinem Zeitpunkt einen Entschlüsselungsschlüssel. Lesen ist ausschließlich Inhabern des Finance-Gruppen-Schlüssels (`vorstand`/`kassierer`) möglich.
