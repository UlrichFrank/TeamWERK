...
Wenn ein Spiel oder Sonstiger Termin gelöscht wird, MÜSSEN alle verknüpften `duty_slots` (und deren `duty_assignments` via vorhandenem CASCADE) automatisch gelöscht werden. Es gibt keinen opt-out. Der Query-Parameter `?delete_slots` SHALL vom Backend ignoriert und vom Frontend nicht mehr gesendet werden.
