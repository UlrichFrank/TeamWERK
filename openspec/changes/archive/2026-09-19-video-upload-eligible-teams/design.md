...
**D3 — `games` nur für die aktive Saison, `game_ids` bleibt saisonübergreifend.**
`game_ids` behält seine Menge (heute: admin alle Spiele, trainer alle Spiele seiner Teams, alle Saisons) — eine Einschränkung wäre eine Vertragsänderung für ältere Tools. Das neue Feld `games` ist dagegen auf die aktive Saison beschränkt: nur dort kann das Tool hochladen (`season_id` der aktiven Saison geht in `POST /api/videos`), und die Antwortgröße bleibt bei admin/vorstand unabhängig von der Vereinshistorie. Das Tool filtert deshalb nur noch auf Datum ≤ heute; `season_id` steht trotzdem im Datensatz, damit ein späterer Client die Menge prüfen kann.
...
- [Response wächst mit Spielanzahl für admin/vorstand] → `games` ist auf die aktive Saison beschränkt (D3), einige hundert Zeilen à ~100 Byte, einmal pro Login; kein Hot-Path. Kein Paging, weil das Tool die volle Menge zum Filtern braucht.
