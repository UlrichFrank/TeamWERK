...
- [x] 1.3 (optional) Beide Hilfsfunktionen als `appdb.SlotInTeamsSQL` nach `internal/db/user_teams.go` zusammenlegen und die Kopien entfernen; nur umsetzen, wenn der Arch-Test (`go test ./internal/arch/...`) grün bleibt, sonst Task als bewusst verworfen markieren
...
- [x] 3.2 `/dienste` einmal im Browser (Chrome DevTools, Performance-Trace) als Admin laden und prüfen, dass nach dem Backend-Fix kein Rendering-Engpass > 500 ms bleibt; Befund notieren und bei Bedarf als eigenen Folge-Change vorschlagen, nicht in diesem Change umsetzen
  - Befund (26.09.2026, Admin, 566 Slots, lokaler Mac): API nach dem Fix ~38 ms, danach **550–630 ms React-Rendering** bis die Liste steht — über der 500-ms-Schwelle. DOM: 18.238 Knoten, 1.847 SVG, 1.761 Buttons (~32 Knoten/Slot); Filter zurück auf „alle" rendert 363 ms ohne Netzwerk. Auf Mobilgeräten ein Mehrfaches. → Folge-Change vorschlagen (Liste begrenzen/„Mehr laden" oder Virtualisierung, Slot-Zeile verschlanken), hier nicht umgesetzt.
  - Nebenbefund (nur lokal, HTTP/1.1): SSE-Streams der vorigen Seiteninstanz bleiben nach einem Reload ~60 s offen und belegen das 6-Verbindungen-Limit; die Navigation fällt nach `networkTimeoutSeconds: 3` (`web/src/sw.ts`) auf den Cache zurück. Prod nutzt HTTP/2 (`deploy/nginx-teamwerk.conf`), dort tritt das Limit nicht auf. Zusätzlich zwei parallele `POST /api/auth/refresh` beim Start (einer 401) — harmlos im Test, aber ein Rotations-Race; beides kein Teil dieses Changes.
- [x] 3.3 Gotcha-Absatz in `docs/agent/06-gotchas.md` ergänzen: korrelierte Team-Prädikate über `game_teams` immer über `game_id` (PK) formulieren, nicht über `team_id IN`; ein Index `game_teams(team_id)` hilft nur ohne `ANALYZE`; verifiziert durch Review des Absatzes
- [x] 3.4 `/verify-change` ausführen (Build, `go test ./...`, Lint, `pnpm -C web build/test/lint`, `openspec validate dienstboerse-ladezeit`); verifiziert durch grünen Lauf
