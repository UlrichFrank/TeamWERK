...
- [x] 1.1 `internal/db/migrations/024_medien_gate.up.sql`:
...
- [x] 1.2 `024_medien_gate.down.sql` — reverse (nur Struktur; Datenverlust
       bei `pending_review`-Zeilen und `medien`-Fkt-Einträgen dokumentieren).
- [x] 1.3 `internal/db/migrations_test.go` bzw. Testutil-Fixtures — neue
       Werte akzeptiert, alte Werte weiter akzeptiert.
- [x] 1.4 Fixtures in `internal/testutil/`: `CreateMedienUser(t, db)`
       (User mit Member + `member_club_functions.function='medien'`).

## 2. Backend — Handler + Guards

- [x] 2.1 `internal/auth/roles.go` — Konstante `ClubFunctionMedien = "medien"`
       ergänzen (Grep-Anker).
- [x] 2.2 `internal/matchreports/handler.go` — neuer Endpoint
...
- [x] 2.3 `internal/matchreports/publish.go` — Guard-Umbau:
...
- [x] 2.4 `internal/matchreports/handler.go` — `PUT /{id}` neu regeln:
       - `state='draft'` → nur Autor darf.
       - `state='pending_review'` → nur Freigeber (medien/vorstand/admin) darf.
       - Sonst 409/403.
- [x] 2.5 `internal/matchreports/handler.go` — neuer Endpoint
...
- [x] 2.6 `internal/matchreports/handler.go` — `GET /{id}` erweitern:
       - Freigeber dürfen `pending_review` lesen (nicht nur autor/published).
- [x] 2.7 `internal/matchreports/handler.go` — Bild-Endpunkte
       (`POST /images`, `DELETE /images/{imgId}`) mit derselben
       State-+Rollen-Matrix wie `PUT /{id}`.

## 3. Notifications

- [x] 3.1 `internal/notifications/` — Helper prüfen; falls fehlt neu bauen:
...
- [x] 3.3 Test: Push-Payload wird an alle Freigeber gesendet
       (Mock-Sender in Testutil).

## 4. Scheduler — Reminder-Job

- [x] 4.1 `internal/scheduler/` — neuer Job
...
- [x] 4.2 Job-Registrierung in `cmd/teamwerk/main.go` (Subcommand
       `scheduler:run` oder Cron-Wrapper — bestehendes Muster).
- [x] 4.3 Test: 6-Tage-alter Bericht → 1 Push; zweiter Job-Lauf →
       0 zusätzliche Pushes (Idempotenz).

## 5. Router-Wiring

- [x] 5.1 `internal/app/router.go` — neue Routen registrieren:
...
- [x] 5.2 Broadcast `match-report-event` in allen neuen Mutations-Routen.

## 6. Frontend

- [x] 6.1 `web/src/lib/api.ts` — Types erweitern:
...
- [x] 6.2 `web/src/pages/MatchReportForm.tsx`:
...
- [x] 6.3 `web/src/pages/MatchReportPendingList.tsx` (neu):
...
- [x] 6.4 `web/src/App.tsx` — Route `/berichte/pruefen` (Guard:
       hasClubFunction `medien` oder `vorstand` oder role=admin).
- [x] 6.5 `web/src/components/AppShell.tsx` — Nav-Eintrag
       „Berichte prüfen" mit Fkt-Gate; Badge für Anzahl offener Pendings.
- [x] 6.6 `useLiveUpdates`-Subscription auf `match-report-event` in
       `MatchReportPendingList` und `MatchReportForm`.

## 7. Tests (Fachlich, nicht Coverage)

- [x] 7.1 Handler-Tests für alle Routen aus proposal.md „Test-Anforderungen":
       Happy-Path + Fehlerfall.
- [x] 7.2 State-Machine-Test: alle erlaubten und verbotenen Übergänge.
- [x] 7.3 Reviewer-Race-Test: 2 gleichzeitige `/publish`-Requests auf
       `pending_review` → genau einer publisht, anderer 409 `in_progress`.
- [x] 7.4 Autor-mit-Medien-Fkt-Test: darf submitten UND freigeben
       (D-2 dokumentiert).
- [x] 7.5 Scheduler-Test: 5-Tage-Reminder feuert einmal, nicht zweimal.
- [x] 7.6 Notification-Test: alle Freigeber-User bekommen 1 Push.

## 8. Konfiguration + Verifikation

- [x] 8.1 `openspec validate spielbericht-medien-gate` grün.
- [x] 8.2 `make build && make test && make lint` grün.
- [x] 8.3 `/verify-change` durchlaufen (Route→Tests, Broadcast/useLiveUpdates,
