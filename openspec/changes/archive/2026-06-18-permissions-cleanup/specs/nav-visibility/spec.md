...
- **THEN** enthält `GET /api/me` → `nav` einen Eintrag mit `route: "/profile"`
...
- **THEN** enthält `GET /api/me` → `nav` KEINEN Eintrag mit `route: "/profile"`
...
- **THEN** enthält `GET /api/me` → `nav` KEINEN Eintrag mit `route: "/members"`

#### Scenario: Admin sieht Mitglieder
- **WHEN** ein User mit `role == "admin"` eingeloggt ist
- **THEN** enthält `GET /api/me` → `nav` einen Eintrag mit `route: "/members"`
