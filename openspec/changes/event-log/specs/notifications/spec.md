...
**Grund:** Ersetzt durch „Push-Fan-out ausschließlich über die Fassade". Die alte Fassung erlaubte Direktaufrufe pauschal „im Scheduler-Job `duty_reminders` und im Authentifizierungs-Pfad" und war damit sowohl unvollständig (sechs reale Bypässe, nicht zwei) als auch nicht mechanisch prüfbar. Die Nachfolge-Anforderung nennt eine Allowlist statt einer Prosa-Ausnahme und wird durch einen Architektur-Test erzwungen. Die drei Löschungs-Szenarien der alten Anforderung sind durch die Capability `event-log` und die Szenarien oben abgedeckt.
