# Design — incremental-list-sync

Herausgelöst aus `incremental-sync`; die Chat-Phase (id-Cursor) ist dort bereits erledigt und hier nicht mehr enthalten.
...
`kader`, `mitfahrgelegenheiten`, `mitfahrt_paarungen` haben `updated_at` bereits → ohne Migration nutzbar (Tier 1).

## Phasenschnitt

1. **`updated_at`-Migrationen** für games/duty_slots/training_sessions + Schreibpfad.
2. **Tombstone-Log** + Delete-Handler + Pruning.
3. **`?since=`** auf games/duty_slots/training_sessions/kader + Frontend-Cursor-Handling.

(Der billigste Tier-1-Einstieg — kader/mitfahrten mit vorhandenem `updated_at` — kann Phase 3 vorgezogen werden, da keine Migration nötig ist.)

## Zusammenspiel mit anderen Changes

```
   scoped-live-updates:  weniger Clients werden getriggert
   incremental-list-sync:  jeder Trigger holt nur das Delta
