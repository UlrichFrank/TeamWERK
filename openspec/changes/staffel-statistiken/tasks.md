...
- [ ] 1.1 Migration `069_bwhv_referees_split` (up/down) legt `bwhv_reports.referees_json TEXT NOT NULL DEFAULT ''` und `referees_uncertain INTEGER NOT NULL DEFAULT 0` an; `referees` bleibt unangetastet. Verifikation: `make migrate-up` läuft durch, `make migrate-down` nimmt beide Spalten zurück, `.schema bwhv_reports` zeigt sie.
