...
### Requirement: `rsvp_opt_out`-Feld (Spiele)

**Reason**: Der Boolean `rsvp_opt_out` konnte nur einen Modus für den Stammkader abbilden und ignorierte den Erweiterten Kader. Ersetzt durch zwei orthogonale Enum-Spalten `rsvp_default_players` und `rsvp_default_extended`, die je drei Modi (`confirmed`/`declined`/`none`) für die beiden Kader-Rollen unabhängig ausdrücken.

**Migration**: Bestandsdaten werden per Migration `018` konservativ übersetzt (`rsvp_opt_out=1` → `players='confirmed'`, sonst `'none'`; `extended='none'` überall). `POST`/`PUT`-Requests mit dem alten Feld `rsvp_opt_out` werden mit HTTP 400 abgelehnt.
