...
- [x] 1.1 `internal/db/migrations/011_event_notes.up.sql` + `.down.sql`:
...
- [x] 1.2 Migrations-Test in `internal/db/migrations_test.go` (falls neu, sonst
      bestehendes Pattern): `011 up` → Spalten/Tabelle vorhanden, CHECK
      verhindert `length(note) > 200`-Insert; `011 down` → Spalten/Tabelle weg.

- [x] 1.3 Aufräum-Logik in den bestehenden DELETE-Handlern (Game/Training):
...
- [x] 2.1 `internal/trainings/handler.go`: neue Methode `UpdateTrainingNote`
...
- [x] 2.2 Tests `internal/trainings/handler_test.go`:
...
- [x] 2.3 `internal/games/handler.go`: neue Methode `UpdateGameNote` analog
...
- [x] 2.4 Tests `internal/games/handler_test.go`:
...
- [x] 3.1 Neuer Job in `internal/scheduler/event_notes_push.go`:
...
- [x] 3.2 Tests `internal/scheduler/event_notes_push_test.go`:
...
- [x] 4.1 `internal/calendar/handler.go`:
...
- [x] 4.2 Tests `internal/calendar/handler_test.go`:
...
- [x] 5.1 `web/src/components/EventNoteIndicator.tsx`: zwei Varianten
...
- [x] 5.2 `web/src/components/EventNoteEditor.tsx`: Textarea (200 max),
...
- [x] 6.1 `web/src/pages/DashboardPage.tsx`: `MeineTermineSection` →
...
- [x] 6.2 `web/src/pages/KalenderPage.tsx`: Game-Tile + Training-Tile —
...
- [x] 6.3 `web/src/components/EventInfoModal.tsx`: bei vorhandenem
...
- [x] 6.4 `web/src/pages/TerminePage.tsx`: Training-Card + Game-Card —
...
- [x] 6.6 `web/src/components/GameEditModal.tsx` +
      ...
      **Entscheidung: Alternative gewählt** — der Hinweis wird ausschließlich
      über `EventNoteEditor` (EventInfoModal + Detailseite) editiert, nicht im
      großen Edit-Modal. Grund: ein großer PUT mit Note-Snapshot würde einen
      via `EventNoteEditor` frisch gesetzten Hinweis überschreiben (Clobber) und
      die Debounce-Queue umgehen. Stattdessen ist `UpdateSession.note` jetzt
      Tri-State (fehlt = unverändert) und `TrainingEditModal` sendet `note`
      nicht mehr mit; `GameEditModal`/`UpdateGame` fassten die Note ohnehin nie an.
      _Commit: `feat(pwa): Hinweis-Feld in Game-/Training-Edit-Modal`_

## 7. Tests-Anpassung Frontend

- [x] 7.1 Existierende Tests für `KalenderPage`/`TerminePage`/
...
- [x] 8.1 `internal/arch/arch_test.go`: prüfen, ob neue Cross-Package-
...
- [x] 8.2 Lint grün: `make lint`, `pnpm -C web lint`.

## 9. Verifikation

- [x] 9.1 `make test` grün (inkl. Architektur-Test, Migrations-Test,
      neue Handler-Tests, Scheduler-Test).
- [x] 9.2 `pnpm -C web test` + `pnpm -C web build` grün.
- [x] 9.3 `openspec validate event-notes --strict` grün.
