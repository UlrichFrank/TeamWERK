...
func testServer(t *testing.T, h *duties.Handler) *httptest.Server {
	...
	return testutil.NewServer(t, func(r chi.Router) {
		...
		r.Get("/api/duty-slots/{id}/comments", h.ListSlotComments)
		r.Put("/api/duty-assignments/{id}/comment", h.SetAssignmentComment)
		r.Delete("/api/duty-assignments/{id}/comment", h.DeleteAssignmentComment)
...
// ── Spielbericht-Slot ohne Rollen-Sonderregel ────────────────────────────────
//
// Der frühere Presseteam-Guard (assertSlotTakePermitted, 403 role_required) ist
// mit der Rolle entfallen: ein Spielbericht-Slot wird gezogen wie jeder andere
// Dienst. Wer ihn zieht, darf anschließend den Bericht schreiben — der Besitz des
// Dienstes IST die Berechtigung.

// matchReportSlot legt einen Spielbericht-Duty-Type + Slot an.
...
func TestClaim_MatchReportSlot_StandardUserOK(t *testing.T) {
	db := testutil.NewDB(t)
	...
	res.Body.Close()
	if res.StatusCode != http.StatusNoContent {
		t.Fatalf("expected 204 für Standard-User, got %d", res.StatusCode)
	}
	if got := slotsFilled(t, db, slotID); got != 1 {
		t.Errorf("slot must be claimed, slots_filled=%d", got)
	}
}

// Der Proxy-Claim durchs Elternteil war der Fall, an dem der Guard die Rolle des
// HANDELNDEN wertete. Ohne Guard bleibt nur der Family-Link-Check — und der lässt
// das Elternteil durch.
func TestClaim_MatchReportSlot_ProxyParentOK(t *testing.T) {
	db := testutil.NewDB(t)
	_, slotID := matchReportSlot(t, db)
	...
	res.Body.Close()
	if res.StatusCode != http.StatusNoContent {
		t.Fatalf("expected 204 (Elternteil mit gültigem Family-Link), got %d", res.StatusCode)
	}
}

// Ohne Family-Link bleibt der Proxy-Claim verboten — der Wegfall des Rollen-Guards
// hat die Stellvertreter-Prüfung nicht mitgenommen.
func TestClaim_MatchReportSlot_ProxyOhneFamilyLinkForbidden(t *testing.T) {
	db := testutil.NewDB(t)
	_, slotID := matchReportSlot(t, db)

	actorUserID := testutil.CreateUser(t, db, "standard")
	otherUserID := testutil.CreateUser(t, db, "standard")
	db.Exec(`UPDATE users SET can_login=0 WHERE id=?`, otherUserID)
	testutil.CreateMember(t, db, otherUserID)

	h := duties.NewHandler(db, testutil.TestConfig(), hub.NewHub())
	srv := testServer(t, h)

	token := testutil.Token(t, actorUserID, "standard", nil)
	res := testutil.Post(t, srv, "/api/duty-board/"+itoa(slotID)+"/claim", token,
		map[string]any{"user_id": otherUserID})
	res.Body.Close()
	if res.StatusCode != http.StatusForbidden {
		t.Fatalf("expected 403 ohne Family-Link, got %d", res.StatusCode)
	}
	if got := slotsFilled(t, db, slotID); got != 0 {
