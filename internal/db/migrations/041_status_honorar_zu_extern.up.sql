-- 041_status_honorar_zu_extern:
--   Benennt members.status-Wert 'honorar' in 'extern' um (klarere Bezeichnung
--   für bezahlte/ehrenamtliche Trainer und andere Vereinshelfer ohne eigene
--   Mitgliedschaft — SQLite kennt kein ALTER … MODIFY CHECK → Tabellen-Rebuild,
--   Muster wie Migration 018/034.
--
...
CREATE TABLE members_new (
    ...
    status                  TEXT     NOT NULL DEFAULT 'aktiv'
                            CHECK (status IN ('aktiv','verletzt','pausiert','ausgetreten','passiv','extern','anwaerter','foerderkind')),
...
SELECT
    id, first_name, last_name, date_of_birth, pass_number, jersey_number,
    position, CASE WHEN status = 'honorar' THEN 'extern' ELSE status END,
