-- Rückweg: rsvp_default_players / rsvp_default_extended → rsvp_opt_out.
-- Datenverlust bei Werten 'declined' (kann der Bool nicht abbilden — mapping
-- auf 0). Werte 'confirmed' auf players werden zu opt_out=1; alles andere zu 0.
-- extended-Voreinstellung wird komplett verworfen.
...
SELECT id, season_id, opponent, date, time, is_home, source, event_type,
    ...
    CASE WHEN rsvp_default_players = 'confirmed' THEN 1 ELSE 0 END,
    rsvp_require_reason, venue_id, end_date, note
FROM games;
...
INSERT INTO training_series_new (id, team_id, season_id, name, location,
    ...
    created_by, created_at, rsvp_opt_out, rsvp_require_reason, venue_id)
