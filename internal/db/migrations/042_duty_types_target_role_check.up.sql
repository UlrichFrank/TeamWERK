-- Aktualisiert den CHECK-Constraint auf duty_types.target_role:
--   Vor 042: ('spieler','elternteil','trainer','admin','vorstand')
--   Nach 042: ('spieler','elternteil','trainer','vorstand','sportliche_leitung','vorstand_beisitzer','kassierer')
--
-- 'admin' wird auf 'vorstand' gemappt (System-Admins sind keine Duty-Zielgruppe).
-- Pre-flight-Hinweis: vor `make migrate-remote-up` einmalig auf der Produktions-DB prüfen:
--   SELECT id, name, target_role FROM duty_types WHERE target_role = 'admin';
-- Falls Treffer → Operator entscheidet, ob das Mapping nach 'vorstand' fachlich passt.
