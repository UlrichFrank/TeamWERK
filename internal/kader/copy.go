...
func copyKader(ctx context.Context, db *sql.DB, fromSeasonID, toSeasonID, targetStartYear int, assignments []CopyAssignment) ([]createdKader, error) {
	...
	for _, a := range assignments {
		// Insert kader for target season
		...
		if newKaderID == 0 {
			...
			tx.QueryRowContext(ctx,
				`SELECT id FROM kader WHERE season_id=? AND age_class=? AND gender=?`,
		...
		case "auto-assign":
			memberCount, err = autoAssignMembers(ctx, tx, int(newKaderID), a.AgeClass, a.Gender, targetStartYear)
			if err != nil {
				return nil, err
			}
...
func autoAssignMembers(ctx context.Context, tx *sql.Tx, kaderID int, ageClass, gender string, seasonStartYear int) (int, error) {
	...
	genderFilter := ""
	var args []any
	args = append(args, kaderID, bracket[0], bracket[1])
	...
	_, err := tx.ExecContext(ctx,
		`INSERT OR IGNORE INTO kader_members (kader_id, member_id)
		 ...
		 WHERE m.status != 'ausgetreten'
		   AND CAST(strftime('%Y', m.date_of_birth) AS INTEGER) BETWEEN ? AND ?`+
			genderFilter,
		args...)
	if err != nil {
		return 0, err
