          ...
          <table className="border-separate border-spacing-0 text-sm">
            <thead>
              <tr>
                ...
                <th scope="col" className="bg-brand-surface-card text-brand-text-muted text-xs uppercase px-4 py-3 text-left border-b border-brand-border-subtle whitespace-nowrap">
                  Teilnahme
                </th>
                {columns.map(i => {
                  const ev = events[i]
              ...
              {members.map((m, rowIdx) => {
                ...
                return (
                  <tr key={m.member_id} className="group">
                    <th
                      scope="row"
                      className={`sticky left-0 z-10 bg-brand-surface-card group-hover:bg-brand-table-select px-4 py-2.5 text-left font-medium text-brand-text border-b border-r border-brand-border-subtle max-w-[9rem] sm:max-w-[14rem] truncate ${groupStart ? 'border-t-2 border-t-brand-border' : ''}`}
                    ...
                    <td className={`px-4 py-2.5 text-brand-text whitespace-nowrap border-b border-brand-border-subtle group-hover:bg-brand-table-select ${groupStart ? 'border-t-2 border-t-brand-border' : ''}`}>
                      {formatParticipation(participation(m, events, columns))}
                    </td>
                    {columns.map(i => {
                      const ev = events[i]
                      ...
                      return (
                        <td
                          ...
                          title={label}
                          aria-label={label}
                          className={`px-2 py-2.5 text-center border-b border-brand-border-subtle group-hover:bg-brand-table-select ${groupStart ? 'border-t-2 border-t-brand-border' : ''}`}
                        >
                          {icon}
                        </td>
                      )
