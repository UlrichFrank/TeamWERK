...
**In-Memory-SQLite gilt pro Verbindung.** Test-Helfer mit `sql.Open("sqlite", ":memory:…")` funktionieren nur, solange alle Zugriffe über dieselbe Pool-Verbindung laufen. Sobald der Code unter Test eine Goroutine startet (`background.Go`, `notify.SendAsync`, Push-Fan-out), zieht diese eine zweite, leere Verbindung und meldet `no such table` — lokal oft zufällig grün, im CI rot. Für solche Tests eine Datei-DB unter `t.TempDir()` verwenden (`testutil.NewDB` oder ein lokaler Helfer wie `newFileTestDB` in `internal/notify`).
