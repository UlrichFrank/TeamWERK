...
- **CLI** (Notfall, falls UI nicht erreichbar):
  ```bash
  ssh teamwerk.team-stuttgart.org "sudo /usr/local/bin/teamwerk maintenance on --db /var/lib/teamwerk/teamwerk.db"
  ssh teamwerk.team-stuttgart.org "sudo /usr/local/bin/teamwerk maintenance off --db /var/lib/teamwerk/teamwerk.db"
  ```
